...
- **False sense of security (거짓 안전감)**
  - 의사 익명화는 개인정보를 흐릴 뿐 **#46 보다 약한 보안 보장**. 가장 큰 문제는 **데이터셋 결합** —
    의사 익명화된 컬럼이 **개인을 식별하는 PII 컬럼으로 되살아날 수 있음**.
...
- **Information loss (정보 손실)**
  ...
  - 여기에 더해 **데이터 타입 손실** 도 있음. **일반화(generalization)** 가 좋은 예 —
    **숫자 값** 이 **텍스트 타입으로 표현된 숫자 범위** 로 바뀜.
...
──────────────────────────────────────────────────────────────────────
 users ─► mapInPandas(pseudo_anonymize_users, users.schema) ─► .withColumn('salary', CASE WHEN) ─► 결과
                     country · ssn 처리 (타입 유지)                    salary 처리 (타입 변경)

 입력 컬럼          변환 경로                       결과값           타입
 ─────────────────────────────────────────────────────────────────────
 user_id  STRING  ─► (변환 없음)                  1                STRING   조인 키로 남김
 country  STRING  ─► mapInPandas · 일반화         eu               STRING   ✓ 타입 유지
 ssn      STRING  ─► mapInPandas · 마스킹         0***-0***-1***   STRING   ✓ 타입 유지
 salary   INT     ─► withColumn · CASE WHEN       "0-50000"        STRING   ⚠ INT → STRING
──────────────────────────────────────────────────────────────────────
 갈림길은 "타입이 바뀌는가" 딱 하나 —
...
──────────────────────────────────────────────────────────────────────
 시각        t0 ─────────── t10 ─────────── t30 ─────────── t31 ─────────── t40
 시크릿 값   pwd_A                          pwd_B (로테이션)
                                              │
 배치 잡     [실행][종료]    [실행][종료]      │    [실행][종료]
             매 실행 시작마다 재조회           │    ✓ 새 실행이 pwd_B 를 자연히 로드
                                              │
 스트리밍 잡 [실행 ────────────────────────────┼──────────────────────► 무기한]
             t0 에 한 번 조회 → 캐시 pwd_A     │    ✗ 캐시가 갱신될 계기가 없음
                                              └─► 다음 연결에서 인증 실패
──────────────────────────────────────────────────────────────────────
 ...
 [대응 세 갈래 — 무엇을 대가로 내는가]
   ① 그냥 실패시키기      재시작 시 새 실행이 재조회 ─► 구현 0 / 자격 증명이 잦으면 실패 폭증
                          ⚠ 재시도로 데이터가 깨지지 않게 멱등성 패턴(챕터 4) 병행 필수
   ② 캐시 TTL 을 짧게     로테이션 주기보다 짧게    ─► 무중단 / 조회 요청·비용 증가
   ③ 비동기 갱신          백그라운드로 재조회       ─► 무중단 / 쓰기 도중 교체되면 실패 가능 ⚠
──────────────────────────────────────────────────────────────────────
 ③의 위험 구간 — 출력 데이터 스토어로 전송을 시작한 뒤, 연결 파라미터를 갱신하기 전 사이.
```
